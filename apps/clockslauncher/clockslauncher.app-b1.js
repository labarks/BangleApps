// Copy of default Launch code to show only Apps in launcher 
// and Clocks app code to run Clocks menu app
...
// Edit Start
var apps = s.list(/\.info$/).map(app=>{var a=s.readJSON(app,1);
return a&&{name:a.name,type:a.type,icon:a.icon,sortorder:a.sortorder,src:a.src};
}).filter(app=>app && (app.type=="app" || (app.type=="clock" && 
settings.showClocks) || !app.type));

// Note: Clock app loads from clocks.info file
// Edit End
